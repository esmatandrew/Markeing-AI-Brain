...
- `api/chat.js` — a serverless function that holds your Gemini API key server-side and proxies the AI Analyst's requests to Google's `generateContent` endpoint. Needed on any real deployment (Vercel, Netlify, etc.) — the browser never calls Gemini directly.
...
3. Get a Gemini API key from [Google AI Studio](https://aistudio.google.com/apikey) if you don't have one.
4. In the Vercel project → **Settings → Environment Variables**, add `GEMINI_API_KEY` with your key, then redeploy so the function picks it up.
5. Done — the dashboard, Opportunity Leaderboard, and AI Analyst panel all work end-to-end. `index.html` already points at `/api/chat` instead of calling Gemini directly.

## Deploy on Netlify instead

Same idea, different function folder. Netlify Functions live under `netlify/functions/` rather than `/api/`, and it reads the key from `GEMINI_API_KEY` the same way via **Site settings → Environment variables**. If you want to go this route, say so and I'll restructure `api/chat.js` into a Netlify Function — the logic itself doesn't change, just the file location and the export format (`exports.handler` instead of Vercel's `export default`).

## Which Gemini model it uses

`api/chat.js` currently calls `gemini-3.5-flash`, Google's current fast general-purpose model with solid tool-calling support. If you'd rather use a different Gemini model (e.g. a more reasoning-heavy one for tougher market analysis), change the `MODEL` constant at the top of `api/chat.js` and redeploy — nothing else needs to change, since `index.html` doesn't know or care which model answers it.
